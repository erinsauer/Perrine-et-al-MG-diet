...
# 
# E1DF <- lme(dailyfood ~ diet*infection*day,
#             random= ~1|ID, correlation = corAR1(),
#                 data=E1.daily.food)
# acf(residuals(E1DF)) 
# # DHARMA temporal autocorrelation results
# simulationOutput <- simulateResiduals(fittedModel = E1DF) 
# # Test for temporal autocorrelation
# residrecal <- recalculateResiduals(simulationOutput, group = E1.daily.food$day)
# testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.daily.food$day))
# 
# summary(E1DF)
# Anova(E1DF)
# E1DFS <- summary(E1DF)
...
E1WF <- lmer(sumfood ~ diet*infection*week + (1|ID), 
             data=E1.weekly.food)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1WF) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.weekly.food$week)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.weekly.food$week))
testResiduals(E1WF)
...
E1ES <- gamm(eyescore ~ Diet+s(Day, k=6), family=nb,
           correlation=corCAR1(form=~Day|Bird), data = E1.ES)
plot(E1ES$gam, by=Diet, pages=1) #this plot lets us visually assess the fit
gam.check(E1ES$gam)
summary.gam(E1ES$gam)
...
E1Mass <- lmer(mass ~ Diet*infection*Day + (1|Bird.ID), 
             data=E1.cond)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Mass) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.cond$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.cond$Day))
testResiduals(E1Mass)
...
E1Fat <- lmer(fat ~ Diet*infection*Day + (1|Bird.ID), 
               data=E1.cond)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Fat) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.cond$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.cond$Day))
testResiduals(E1Fat)  
...
E1FatMG <- lme(fat ~ Diet*Day, random=~ 1|Bird.ID, 
               data=E1.condMG)
plot(ACF(E1FatMG))
summary(E1FatMG)
Anova(E1FatMG)
...
E1AB <- glmer(AB ~ diet*Day + (1|Bird.ID), 
                 family=gaussian, data=E1.AB)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1AB) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.AB$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.AB$Day))
testResiduals(E1AB) 
...
E1Load <- lmer(log_load ~ Diet*day + (1|Bird), 
                  data=E1.load)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Load) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.load$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.load$Day))
testResiduals(E1Load)
...
# E1Bas <- glmer(Basophil ~ Diet*Infection*Day + (1|Bird.ID), 
...
E1Eos <- glmer(Eosinphil ~ Diet*Infection*Day + (1|Bird.ID), 
                 family=poisson, data=E1.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Eos) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.WBC$Day))
testResiduals(E1Eos)
...
E1Het <- lmer(Heterophil ~ Diet*Infection*Day + (1|Bird.ID), 
                  data=E1.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Het) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.WBC$Day))
testResiduals(E1Het)
...
E1Lym <- lmer(Lymphocyte ~ Diet*Infection*Day + (1|Bird.ID), 
                 data=E1.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Lym) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.WBC$Day))
testResiduals(E1Lym)
...
E1Mon <- glmer(Monocyte ~ Diet*Infection*Day + (1|Bird.ID), 
                 family=poisson, data=E1.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Mon) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.WBC$Day))
testResiduals(E1Mon)
...
E1HLR <- glmer(cbind(Heterophil,Lymphocyte) ~ Diet*Infection*Day+
                   (Day|Bird.ID),  
                 family=binomial,data=E1.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1HLR) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.WBC$Day))
testResiduals(E1HLR)
...
E1Hem <- lmer(Hem ~ diet*infection*Time.Point + (1|Bird.ID), 
                  data=E1.hem)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E1Hem) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E1.hem$Time.Point)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E1.hem$Time.Point))
testResiduals(E1Hem) 
...
ggplot(E1.hem, aes(y=Hem, x=Time.Point, color=diet)) +
  ...
  ylab("Hematocrit")
########### Days to recovery ###############
E1.Recovery <- read.csv("2020 DietXDisease_recovery.csv")
str(E1.Recovery) 
hist(E1.Recovery$Day)

E1Reco <- coxph(Surv(Day, recovery) ~ Diet, data=E1.Recovery)
cox.zph(E1Reco)

summary(E1Reco)

fit <- survfit(Surv(Day, recovery) ~ Diet, data=E1.Recovery)
print(fit)
...
#               data=E2.daily.food)
# # DHARMA temporal autocorrelation results
# simulationOutput <- simulateResiduals(fittedModel = E2DFP) 
# # Test for temporal autocorrelation
# residrecal <- recalculateResiduals(simulationOutput, group = E2.daily.food$Day)
# testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.daily.food$Day))
# 
# summary(E2DFP)
# Anova(E2DFP)
...
#               data=E2.daily.food)
# # DHARMA temporal autocorrelation results
# simulationOutput <- simulateResiduals(fittedModel = E2DFT) 
# # Test for temporal autocorrelation
# residrecal <- recalculateResiduals(simulationOutput, group = E2.daily.food$Day)
# testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.daily.food$Day))
# 
...
E2WFL <- lmer(SumLipid ~ sex*infection*Week + (1|ID), 
             data=E2.weekly.food)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2WFL) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.weekly.food$Week)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.weekly.food$Week))
testResiduals(E2WFL)
...
E2WFP <- lmer(SumProtein ~ sex*infection*Week + (1|ID), 
              data=E2.weekly.food)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2WFP) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.weekly.food$Week)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.weekly.food$Week))
testResiduals(E2WFP)
...
E2WFT <- lmer(SumTotal ~ sex*infection*Week+
             (Week|ID), #correlation = corAR1(form=~Week|ID),
              data=E2.weekly.food)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2WFT) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.weekly.food$Week)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.weekly.food$Week))
testResiduals(E2WFT)
...
E2ES <- gamm(ES ~ Sex+s(Day, k=6), family=nb,
...
E2Mass <- lmer(mass ~ sex*Infection*Day + (1|Bird), 
               data=E2.cond)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Mass) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.cond$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.cond$Day))
testResiduals(E2Mass)
...
E2Fat <- lmer(fat ~ sex*Infection*Day + (1|Bird), 
              data=E2.cond)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Fat) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.cond$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.cond$Day))
testResiduals(E2Fat)
...
E2AB <- glmer(AbValue ~ Sex*Day + (1|BirdID), 
                family=gaussian, data=E2.AB)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2AB) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.AB$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.AB$Day))
testResiduals(E2AB)
...
E2Load <- glmer(log.load ~ sex*day + (1|Bird.ID), 
                  family=gaussian, data=E2.load)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Load) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.load$day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.load$day))
testResiduals(E2Load)
...
E2Eos <- lmer(Eosinphil ~ sex*trt*Day + (1|Bird.ID), 
                  data=E2.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Eos) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2Eos)
...
E2Het <- glmer(Heterophil ~ sex*trt*Day + (1|Bird.ID), 
                 family=poisson, data=E2.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Het) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2Het)
...
E2Lym <- lmer(Lymphocyte ~ sex*trt*Day + (1|Bird.ID), 
                 data=E2.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Lym) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2Lym)
...
E2Mon <- glmer(Monocyte ~ sex*trt*Day + (1|Bird.ID), 
                 family=poisson, data=E2.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Mon) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2Mon)
...
E2HLR <- glmer(cbind(Heterophil,Lymphocyte) ~ sex*trt*Day + (1|Bird.ID), 
                 family=binomial, data=E2.WBC)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2HLR) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2HLR)
...
E2Hem <- lmer(Hematocrit ~ sex*Treatment*Time.Point + (1|Bird.ID), 
                 data=E2.hem)
# DHARMA temporal autocorrelation results
simulationOutput <- simulateResiduals(fittedModel = E2Hem) 
# Test for temporal autocorrelation
residrecal <- recalculateResiduals(simulationOutput, group = E2.WBC$Day)
testTemporalAutocorrelation(simulationOutput = residrecal, time = unique(E2.WBC$Day))
testResiduals(E2Hem)
